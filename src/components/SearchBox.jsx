  ...
  return (
    <>
      <div className="flex overflow-clip border-2 p-2 gap-2 items-center justify-between " style={{color:store!=="Dark"?"black":"White",backgroundColor:store!=="Dark"?"White":"black"}}>
        ...
        <input type="text" className="border-2 w-[50%]" />
        
        <div
          name="name"
          className="border-2 flex flex-wrap items-center gap-1 hover:cursor-pointer"
          onMouseEnter={() => {
            setVal("block");
          }}
          onMouseLeave={() => {
        ...
        <div id="cart" title="Wishlist" className='hover:cursor-pointer'>
          <img
        ...
        <div id="cart" title="Help-Links" className='hover:cursor-pointer'>
          <img
            src="https://static-assets-web.flixcart.com/batman-returns/batman-returns/p/images/header_3verticalDots-ea7819.svg"
            alt=""
          />
        </div>
        <button onClick={() => {changeMode()}} className='hover:cursor-pointer'>{store} Mode</button>
      </div>
    </>
  );
}
